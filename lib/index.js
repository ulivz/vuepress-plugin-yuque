...
const { hash, logger, chalk, parseFrontmatter, inferTitle } = require('@vuepress/shared-utils')
...
module.exports = (opts, ctx) => {
  ...
  return {
    ...
    chainMarkdown(config) {
      // Disable html for now.
      // Many yuque writers would write raw HTML in the markdown
      // Only these markup under inline code(`) will be transpiled successfully
      config
        .options
        .html(false)
    },
    ...
    async ready() {
      ...
      if (!(repoDetail && repoDetail.data && repoDetail.data.name)) {
        spinner.fail(`Please check your repoId: ${chalk.yellow(repoId)}, response: ${JSON.stringify(repoDetail)}`)
        process.exit(1)
      }
      spinner.succeed(`Retrieved repo detail`)

      const { name, description } = repoDetail.data
      spinner.succeed(`Your Yuque website: ${chalk.green(name)} ${chalk.gray(description)} ...`)
